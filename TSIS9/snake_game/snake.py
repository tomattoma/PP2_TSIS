...
while not done:
    ...
    if pos is not None:
        worm.increase(pos)
        score += random.randint(10,30)  # Увеличиваем счет на 10 за каждую съеденную еду
        if len(worm.points) % 3 == 0:
            wall.next_level()
            level += 1
    food.update_timer()  # Чтобы еда исчезала со временем
    clock.tick(3 + level)  # Чтобы увеличивать скорость со временем
